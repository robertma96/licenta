...
def my_profile(request):
    current_uder_id = request.user.id
    user_objects = UserStyling.objects.filter(user_id=current_uder_id)
    my_pictures = ['media' + element.resulted_image.split('media')[1] for element in user_objects]
